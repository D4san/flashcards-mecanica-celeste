...
const BOSS_IMAGE_SRC = "/boss.webp"
  ...
  {
    "question": "¿Por qué es fundamental para la mecánica celeste que los sistemas astrofísicos reales (como sistemas planetarios o estelares múltiples) experimenten un \"tiempo de relajación\", y cómo esto justifica que estudiemos el problema de los 2-cuerpos en lugar de integrar matemáticamente el caos absoluto?",
    "answer": "Durante el \"tiempo de relajación\", un sistema colisional y caótico de $N$ cuerpos redistribuye su energía a través de encuentros gravitacionales. Esto causa que se expulsen componentes o que los cuerpos se agrupen en \"pares\" separados por distancias inmensas o dominados por un cuerpo masivo, convirtiéndose en un sistema organizado o jerárquico. Al autoorganizarse en $N-1$ pares, la astrofísica puede abandonar la abrumadora tarea de resolver $N$ interacciones simultáneas y enfocarse en aislar el comportamiento matemático de sistemas de dos cuerpos."
  },
  {
    "question": "Las transformaciones de las coordenadas de Jacobi permiten recuperar las posiciones absolutas espaciales de dos cuerpos a partir de su centro de masa y su vector relativo. En la ecuación $\\vec{r}_1 = \\vec{R}_{CM} + \\frac{m_2}{M}\\vec{r}$, ¿cuál es el significado físico profundo de que la posición del cuerpo 1 dependa de la masa del cuerpo 2 ($m_2$) dividida por la masa total del sistema ($M$)?",
    "answer": "Este factor actúa como un \"peso\" gravitacional que dicta la dinámica espacial. Físicamente refleja que la distancia de un objeto a su centro de masa común es inversamente proporcional a su propia masa. Por ejemplo, la Tierra orbita muy cerca de su centro de masa con la Luna, porque el factor multiplicador que fija su distancia es la diminuta masa de la Luna dividida por la masa total. Este es el \"pegante\" que ata proporcionalmente el vector relativo al espacio físico."
  },
  {
    "question": "La primera cuadratura deducida es el Momentum Angular Relativo Específico (M.A.R.E. o $\\vec{h} = \\vec{r} \\times \\dot{\\vec{r}}$). Dado que es un vector inmutable, ¿qué restricción espacial inquebrantable le impone la dirección constante de $\\vec{h}$ al movimiento de dos cuerpos celestes a lo largo de millones de años?",
    "answer": "Por la propia definición geométrica del producto cruz, el vector $\\vec{h}$ es y será siempre perpendicular a los vectores de posición y velocidad. Al ser $\\vec{h}$ una constante absoluta del movimiento (su dirección no cambia nunca), impone que el vector relativo $\\vec{r}$ y la velocidad jamás puedan abandonar el plano perpendicular a $\\vec{h}$. Esto demuestra que en el universo, el problema de los dos cuerpos ocurre y está atrapado estrictamente en un único plano fijo (es coplanar)."
  },
  {
    "question": "La cuadratura de la Energía Relativa Específica ($\\epsilon = \\frac{1}{2}v^2 - \\frac{\\mu}{r}$) tiene el formato de la energía cinética y potencial, pero omite explícitamente la masa del cuerpo menor que está orbitando. ¿Por qué en astrofísica es tan útil trabajar con esta \"energía por kilogramo\"?",
    "answer": "Trabajar con esta \"energía sin masa\" es sumamente útil porque demuestra una regla fundamental del universo: el comportamiento dinámico de una órbita no depende de lo pesado que sea el objeto que viaja por ella. Al no incluir la masa del objeto menor, esta fórmula nos permite calcular umbrales universales que aplican para cualquier cosa. El mejor ejemplo es la velocidad de escape. En la clase, al igualar esta energía a cero, el profesor calculó que la velocidad de escape de la Luna es de aproximadamente 2.4 km/s. Gracias a que la fórmula no tiene la masa del objeto, sabemos inmediatamente que esos 2.4 km/s aplican para todo: es la misma velocidad que necesita alcanzar un pequeño módulo lunar frágil (una \"lata de cerveza\", como en las misiones Apolo), un astronauta saltando, o un asteroide gigante para poder escapar de la gravedad lunar. En resumen: Sirve porque nos permite saber qué le pasará a un cuerpo en el espacio fijándonos únicamente en la fuerza del planeta o estrella anfitriona (su gravedad $\\mu$ y su radio $r$), simplificando enormemente los cálculos."
  },
  {
    "question": "Descomponiendo la velocidad mediante la fórmula geométrica en coordenadas cilíndricas, se determina que la velocidad tangencial equivale a $h/r$. Conceptualmente, ¿qué nos indica esta relación inversa sobre la dinámica de un objeto celeste a medida que cae o se aleja en un pozo gravitacional estelar?",
    "answer": "Nos indica matemáticamente que, dado que el momentum angular específico ($h$) es una constante inquebrantable, la única forma de equilibrar la ecuación es que cuando el objeto reduce su distancia ($r$) al periapsis (se acerca al centro), su velocidad en la tangente orbital debe compensatoriamente aumentar (se acelera). Inversamente, al alcanzar mayores distancias, el objeto debe de frenarse de forma proporcional. Estar cerca implica ir obligatoriamente más rápido.",
    "nanswer": "Nos indica matemáticamente que la velocidad total del objeto decae de forma estrictamente inversamente proporcional a la distancia en cualquier punto de la órbita ($v = h/r$). Esto significa que si el objeto duplica su distancia a la estrella anfitriona, su velocidad de traslación espacial se reduce exactamente a la mitad, conservando así el equilibrio de la energía cinética y potencial del sistema gravitacional de forma perfectamente simétrica.",
    "isTrap": true
  },
  {
    "question": "Sabemos matemáticamente que una ecuación diferencial con 6 variables de estado requiere solo 6 constantes (cuadraturas) para ser resuelta. No obstante, al derivar la constante de Laplace ($\\vec{e}$), el sistema llega a sumar 7 constantes (3 del $\\vec{h}$, 1 de la energía y 3 del $\\vec{e}$). ¿Cómo se evita que el sistema entre en contradicción por sobreabundancia de datos?",
    "answer": "La sobreabundancia de información es solo aparente, porque las cuadraturas vectoriales no son absolutamente independientes entre sí. Por sus definiciones geométricas mediante el producto cruz, el vector de Laplace siempre habita el mismo plano orbital que la partícula. Simultáneamente, sabemos que el M.A.R.E. ($\\vec{h}$) atraviesa ortogonalmente (a 90°) ese plano. Esta perpendicularidad obligatoria fuerza matemáticamente a que su producto escalar sea nulo ($\\vec{h} \\cdot \\vec{e} = 0$), actuando como una relación o restricción geométrica de dependencia que anula la séptima incógnita y estabiliza el modelo en exactamente 6 ecuaciones algebraicas resolubles.",
    "nanswer": "La sobreabundancia de información se soluciona porque el vector de Laplace ($\\vec{e}$) y el vector del momentum angular ($\\vec{h}$) son colineales en el espacio tridimensional. Dado que la excentricidad dicta la estabilidad de la órbita y el momento angular dicta su rotación, ambos vectores deben apuntar perpendicularmente hacia afuera del plano orbital para conservar la invariancia del sistema. Esto hace que el producto cruz entre ellos sea nulo ($\\vec{h} \\times \\vec{e} = 0$), reduciendo matemáticamente el sistema de 7 a 6 constantes independientes.",
    "isTrap": true
  },
  {
    "question": "La fórmula analítica descubierta al proyectar la geometría (la ecuación de la trayectoria de las cónicas) representa un hito gigantesco para la física analítica en comparación al postulado histórico planteado por Kepler en su Primera Ley. ¿Cuál es el significado epistemológico y teórico de derivar esta solución frente al modelo de Kepler?",
    "answer": "Kepler asumió las trayectorias elípticas como un principio de partida heurístico, adivinado a prueba y error porque la forma calzaba elegantemente con los datos observacionales. Sin embargo, mediante el cálculo de Laplace y la resolución del vector de excentricidad proyectado sobre $\\vec{r}$, la elipse se devela teóricamente no como una suposición inicial, sino como el resultado puramente algebraico final e irremediable del problema de cuadraturas gravitacionales. La cónica es un derivado formal de la física de atracción a la inversa del cuadrado."
  }
]
