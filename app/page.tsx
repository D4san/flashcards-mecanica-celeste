...
const BOSS_IMAGE_SRC = "/boss.webp"
  ...
  {
    "question": "Si la anomalía media ($M$) describe un ángulo de un cuerpo \"ficticio\" que se mueve a velocidad angular constante, ¿por qué necesitamos inventar este concepto para predecir la posición de un planeta real?",
    "answer": "Porque el tiempo avanza de manera constante, pero los planetas reales barren su órbita a velocidades variables (más rápido cerca del periapsis, según la ley de áreas). La anomalía media crea un puente matemático directo con el tiempo transcurrido ($M = n(t-t_0)$). A través de la ecuación de Kepler, podemos comparar y conectar la posición de este cuerpo de velocidad idealizada con la anomalía excéntrica ($E$) que sí nos permite trazar la geometría de la órbita real del planeta."
  },
  {
    "question": "Kepler dedujo su tercera ley de manera empírica, relacionando los períodos y los semiejes de los planetas. Cuando Newton dedujo esta misma ley usando la mecánica clásica, ¿qué gran revelación física le aportó al teorema armónico que Kepler no podía saber?",
    "answer": "Newton demostró que la constante empírica de proporcionalidad de Kepler no era un número mágico arbitrario, sino que estaba directamente determinada por el parámetro gravitacional ($\\mu$), es decir, la masa total del sistema interactuante (por ejemplo, el Sol más el planeta). Esto le dio un sentido universal a la ley gravitatoria y probó que la cantidad de masa es lo que rige el \"reloj\" (período) del movimiento.",
    "nanswer": "Newton demostró que la constante empírica de Kepler no era arbitraria, sino que a través de ella se podía deducir por primera vez el valor exacto de la Constante de Gravitación Universal ($G$). Esto le permitió cuantificar la fuerza absoluta no solo de los planetas, sino de cualquier par de masas interactuantes en el universo a través de la ecuación $F = GmM/r^2$.",
    "isTrap": true
  },
  {
    "question": "El teorema armónico dictamina que el período de una órbita depende del semieje mayor pero es independiente de su excentricidad ($e$). Físicamente, ¿qué implica esto al comparar una órbita perfectamente circular y una órbita muy alargada que posean el mismo semieje mayor?",
    "answer": "Implica que a pesar de que el cuerpo en la órbita alargada experimenta aceleraciones extremas (viajando sumamente rápido cerca del periapsis y lentísimo en el apoapsis), estas variaciones se compensan perfectamente a lo largo del trayecto. Al final, su velocidad angular promedio (el movimiento medio, $n$) será igual al de la órbita circular, haciendo que ambos cuerpos tarden exactamente el mismo tiempo en dar una vuelta completa al foco."
  },
  {
    "question": "Al calcular numéricamente la posición del asteroide Apophis para el año 2029 asumiendo la teoría de los dos cuerpos, se encontraron decenas de miles de kilómetros de desfase frente a la simulación del JPL. Conceptualmente, ¿qué falla en la idealización matemática de los dos cuerpos?",
    "answer": "Falla que asume un universo aislado. A largo plazo, se acumulan las perturbaciones gravitacionales ejercidas por cuerpos terciarios (como la fuerza de Júpiter u otros asteroides sobre la órbita). La fuerza neta sobre un cuerpo espacial no depende de un solo baricentro, demostrando que el \"problema de los 2 cuerpos\" es un modelo matemático aproximado a corto plazo, pero en el tiempo real, la mecánica espacial es un innegable sistema complejo de $N$-cuerpos interactuando continuamente."
  },
  {
    "question": "El formalismo $f$ y $g$ permite predecir el futuro vector de posición o velocidad de un objeto exclusivamente combinando linealmente sus vectores de posición y velocidad iniciales, sin derivar la rotación de la órbita. ¿Cuál es el principio puramente geométrico que permite que esto sea válido?",
    "answer": "Conceptualmente se basa en el principio de que todo el movimiento de un problema ideal de dos cuerpos ocurre estrictamente contenido dentro de un plano invariable a lo largo del tiempo. De acuerdo al álgebra vectorial, cualquier vector que yazca dentro de un plano bidimensional siempre podrá expresarse como una combinación lineal (multiplicado por coeficientes escalares, en este caso las funciones $f$ y $g$) de otros dos vectores fundamentales en ese mismo plano."
  },
  {
    "question": "En los sistemas jerárquicos de 3 cuerpos (como el Sol, la Tierra y un satélite o el JWST), las masas y las distancias están tan escalonadas que a corto plazo parecen sistemas estables modelables mediante la simple suma del comportamiento de \"pares\" de dos cuerpos. Pero conceptualmente, ¿qué propiedad intrínseca de los N-cuerpos destruye este orden teórico si el tiempo corre lo suficiente?",
    "answer": "Emerge el comportamiento caótico, que físicamente representa una sensibilidad extrema del modelo dinámico a sus condiciones iniciales. Sin importar cuán jerárquico parezca el sistema, la mínima desviación y la acumulación continua de perturbaciones en el largo plazo hará que las predicciones analíticas de los 2 cuerpos diverjan por completo o fallen \"miserablemente\", destruyendo la predictibilidad exacta."
  },
  {
    "question": "Salvo en la periapsis o apoapsis, matemáticamente se cumple que la Anomalía Excéntrica ($E$) es mayor que la Anomalía Media ($M$) en la primera mitad del recorrido orbital. Físicamente, ¿qué significa esta desigualdad si pensamos en el concepto de conservación de velocidad areal?",
    "answer": "Significa que el planeta real \"se adelanta\" a donde debería estar si fuese a una velocidad constante. Al pasar por el periapsis, el planeta está más cerca del foco y viaja a mucha mayor velocidad para conservar su momento angular y barrer la misma área. Por ello, su ángulo geométrico que describe la posición ($E$) abarca más trayecto que el ángulo promedio imaginario dictado por un cronómetro ideal de velocidad constante ($M$).",
    "nanswer": "Significa que, debido a la conservación del momento angular, el planeta debe barrer áreas iguales en tiempos iguales según la segunda ley de Kepler. Por lo tanto, al acercarse al periapsis viaja más rápido, haciendo que el ángulo real de su posición medido desde el Sol (la Anomalía Excéntrica, $E$) crezca mucho más rápido que el ángulo del cuerpo ficticio de velocidad constante ($M$).",
    "isTrap": true
  },
  {
    "question": "Al introducir un modelo de $N$ cuerpos unidos (como un péndulo estricto con una barra fija), Newton nos pediría conocer las complejas fuerzas de tensión internas. Conceptualmente, ¿qué visión de la física ofreció Lagrange (con los trabajos virtuales) para poder ignorar y eliminar matemáticamente estas molestas fuerzas internas?",
    "answer": "La innovación conceptual de Lagrange fue abstraer el problema. En lugar de tratar de resolver el rompecabezas vector por vector sumando tensiones, propuso encontrar variables de movimiento que expresen el sistema únicamente a través de los desplazamientos admisibles y las fuerzas efectivas que generan movimiento real. Esto permite que el formalismo cancele automáticamente las fuerzas de restricción (como la fuerza que restringe el estiramiento de una cuerda o barra) permitiendo resolver el sistema a partir de las energías y los desplazamientos virtuosos admisibles, desapareciendo la necesidad de calcular fuerzas inútiles."
  }
]
